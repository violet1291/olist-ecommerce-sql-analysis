...
-- In this dataset, `customer_id` does not uniquely identify a customer. Therefore, `customer_unique_id` is used to calculate the repurchase rate.
...
SELECT 
 ...
 ROUND(SUM(orders.price), 2) AS value_total
  FROM olist_orders_dataset  o
  JOIN olist_customers_dataset cus ON o.customer_id = cus.customer_id
  ...
  ROUND(SUM(value_total),2) AS total_spent
  FROM type_customers
...
WITH orders_city AS (
	...
	AND ordat.order_approved_at IS NOT NULL
    GROUP BY   
